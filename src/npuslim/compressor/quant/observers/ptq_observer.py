...
class PTQObserver(nn.Module, metaclass=abc.ABCMeta):
    def __init__(
        ...
        **kwargs,
        ...
        quant_algo_info = QuantConfigManager.get_config()
        ...
        if act_observer:
            ...
            self.act_observer = act_observer(
                self.layer, quant_bits=quant_algo_info.a_quant_bits, **kwargs
            )
        ...
        if kv_cache_observer:
            ...
            self.kv_cache_observer = kv_cache_observer(
                self.layer,
                quant_bits=quant_algo_info.c_quant_bits,
                quantization_wise=quant_algo_info.quant_algo,
            )
        if weight_observer:
