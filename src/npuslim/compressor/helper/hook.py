# Adapted from Neuralmagic's llmcompressor: 
...
if __name__ == "__main__":
    ...
    class AscendNPUDebugger(HooksMixin):
        _tensor_stats: dict = PrivateAttr(default_factory=dict)

        def trace_hook(self, module, inp, out):
            layer_name = module.__class__.__name__
            has_nan = torch.isnan(out).any().item()
            print(f"[NPU-TRACE] 层: {layer_name}")
            print(f"输出形状: {list(out.shape)} | 是否有 NaN: {has_nan}")
            self._tensor_stats[layer_name] = out.detach().cpu()

    debugger = AscendNPUDebugger()
    model = SimpleLLM()
    input_data = torch.randint(0, 10, (1, 8))

    for name, m in model.named_modules():
        if isinstance(m, nn.Linear):
            debugger.register_hook(m, debugger.trace_hook, "forward")
    model(input_data)
