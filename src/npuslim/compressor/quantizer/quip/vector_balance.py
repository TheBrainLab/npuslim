...
def ldlp_admm(H, rho=0.1, niters=100):
    ...
    for ii in range(100):
        ...
        C = torch.diag(
            1
            / torch.max(
                torch.tensor(1.0, device=H.device), ((X + W).T @ (X + W)).diag().sqrt()
            )
        )
        Z = (X + W) @ C
        W = W + X - Z
        objective = (
            (Z + torch.eye(n, device=H.device))
            @ H
            @ (Z + torch.eye(n, device=H.device)).T
        ).trace()
...
def round_ldl_admm(w, H, nbits, n_greedy_passes=9, unbiased=False):
    ...
    for jj in range(n_greedy_passes):
        ...
        for ii in range(d):
            wr_prev = wr.clone()
            wr = torch.clamp(
                torch.round(wr_target + (w - wr) @ HnM), min=0, max=2**nbits - 1
            )
            if (wr == wr_prev).all():
...
def round_vecbal_Hsort(
    ...
    # 1. Dispatch dictionary for methods that have a standard and lazy_batch variant
    # Format: 'method_name': (standard_function, block_function)
    method_registry = {
    ...
    # 2. Handle standalone methods early
    if qmethod == "ldlbal_admm":
    ...
    # 3. Handle registry methods
    if qmethod in method_registry:
        std_fn, block_fn = method_registry[qmethod]
        target_fn = block_fn if lazy_batch else std_fn
        ...
        if qmethod == "allbal":
            ...
            # Note: allbal uses 'npasses' instead of 'n_greedy_passes'
            wp_hat = target_fn(
                wp,
                Hp,
                nbits=nbits,
        ...
        # 'ldlq' and 'ldlqRG' cast weights to float
        return target_fn(
            w.float(), H, nbits=nbits, n_greedy_passes=npasses, unbiased=unbiased
        )

    # 4. Fallback if an unknown method is provided
    raise ValueError(f"Unknown qmethod: {qmethod}")
...
def quantize_weight_vecbal(
    ...
    """
    # Early exit for the specific GPTQ equivalent edge case
    if qfn == "a" and qmethod == "ldl_gptqequiv":
        wr = round_ldl_gptqequiv((w / scale) + zero, H, nbits=nbits)
        # Note: intentionally not returning wr.half() here to match original logic
        return scale * (wr - zero)

    # --- Phase 1: Pre-processing based on quantization function ---
    if qfn == "a":
        wr = torch.clamp((w / scale) + zero, 0, maxq)
    elif qfn == "b":
        scale_b = 2.4 * w.square().mean().sqrt() + 1e-16
        wr = w / scale_b
        wr = torch.clamp(((wr + 1) / 2) * maxq, 0, maxq)
    else:
        # Fixed bug: use raise instead of return
        raise NotImplementedError(f"Quantization function '{qfn}' is not supported.")

    # --- Phase 2: Common routing logic ---
    ...
    # --- Phase 3: Post-processing based on quantization function ---
    if qfn == "a":
        wr = scale * (wr - zero)
    elif qfn == "b":
        wr = (wr / maxq) * 2 - 1
