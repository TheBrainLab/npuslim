...
class SlimEngine:
    def __init__(self):
        ...
        self.saver = SlimSaver(
            save_path=self.cfg.meta.save_path, 
            config=self.cfg
        )
    ...
    def prepare_dataloader(self):
        ...
        dataset = DatasetFactory.create(
            processor=processor, 
            config=self.cfg.calib_dataset.dataset
        )
    ...
    def build_pipeline(self):
        ...
        if hasattr(self.cfg, "sparse") and self.cfg.sparse:
            ...
            self.pipeline.append(SparseTask(
                model=self.model,
        ...
        if hasattr(self.cfg, "speculative") and self.cfg.speculative:
            self.pipeline.append(SpeculativeTask(
                target=self.model,
    ...
    def get_ignore_layers(self, compressor: str = "ptq"):
        ...
        logger.info(f"Layers ignored during {compressor.upper()}: {layers_str}")
        return final_ignore
